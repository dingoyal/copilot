...
# wrie log to a file
# logger.basicConfig(filename='data_scientist.log',level=logging.DEBUG)
...
1. You first need to identify the list of usable tables, You only have to select from DAUReport & UGBReport tables. 
2. From the question, you decide on which tables are needed to acquire data
3. Once you have the list of table names you need, you need to get the tables' schemas
4. Then you can formulate your SQL query, If time frame is not provided try to pick latest 2 years of data. Schema contains monthly time-series data at Geo, Segment level. It has Historical Actual and Future Forecast columns. Actual data is present till today and Forecast is present for next 2 years.
5. Check your data 
...
Thought: I now have the schema of the tables I need. I am ready to build query to retrieve data. If time range is not provided try to filter past 2 years of data. Schema contains monthly time-series data at Geo, Segment level. It has Historical Actual and Future Forecast columns.
Action: 
```python
...
class SQL_Data_Preparer(ChatGPT_Handler):
    ...
    def run(self, question: str, show_code,show_prompt,st) -> str:
        ...
        def test(python_code):
            try:
                exec(python_code, locals())
                return "successully run"
        ...
        while count<= max_steps:
            # logger.info("((((((((((((((((((((((((((((((((((((((()))))))))))))))))))))))))))))))))))))))")
            st.write(f"data engineer user_question: {user_question}" )
            # logger.info(f"data engineer new_input: {new_input}" )
            # st.write(f"data engineer new_input: {new_input}")
            llm_output,next_steps = self.get_next_steps(user_question= user_question, assistant_response =new_input, stop=["Observation:"])
            logger.info(f"data engineer next_steps: {next_steps}" )
            st.write(f"data engineer llm_output: {llm_output}" )
            logger.info("((((((((((((((((((((((((((((((((((((((()))))))))))))))))))))))))))))))))))))))")
            user_question=""
            ...
            if not run_ok:
                st.write(f"encountering error: {error_msg}, \nI will now retry data engineering")

            count +=1
            # st.write("llm_output: "+llm_output)
            if "Final Answer:" in llm_output:
                final_output= output.get("comment","")+output.get("text_after","")+output.get("text_after","")
                return final_output
            if count>= max_steps:
                # st.write(run_ok)
                st.write("Data Engineer: I am sorry, I cannot handle the question, please change the question and try again")
