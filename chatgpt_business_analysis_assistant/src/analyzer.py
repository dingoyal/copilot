...
# logger.basicConfig(filename='analyzer.log',level=logging.DEBUG)
    ...
    def get_next_steps(self, updated_user_content, stop):
        ...
        try:
            ...
            if "maximum context length" in str(e):
                logger.info(f"Context length exceeded")
                return "OPENAI_ERROR",""  
            ...
            while n<5:
                try:
                    ...
                    logger.info(f"error calling open AI, I am retrying 5 attempts , attempt {n}")
                    time.sleep(8) #sleep for 8 seconds
                    logger.info(str(e))
    ...
    def python_run(self, question: str, show_code,show_prompt,st) -> any:
        ...
        while count<= max_steps:
            ...
            if run_ok:
                ...
                st.write(f"encountering error: {error_msg}, \nI will now retry analyses")

            count +=1
            if count>= max_steps:
                st.write("Analyzer: I am sorry, I cannot handle the question, please change the question and try again")
